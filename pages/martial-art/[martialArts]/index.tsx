...
const MartialArtPage = ({ data }) => {
  const { query } = useRouter();
  return (
    <>
      <Head>
        <title>
        {data?.length ?? ''} {query?.martialArts} Across the U.S.{" "} |  DOJO+
        </title>
        <meta
...
export const getServerSideProps: GetServerSideProps = async ({ params }) => {
  const { martialArts } = params;
  try {
    const baseUrl = `${BASE_URL}api/Schools/get-school-basedon-martialart`;
